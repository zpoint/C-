...
int main(int argc, char **argv) 
{
  ...
  float rate = 0;
  bool random_flag = true, label_flag = true, sort_by_length = true;
  ...
  if ((i = ArgPos("-sort", argc, argv, false)) > 0) sort_by_length = bool(atoi(argv[i + 1]));
  if ((i = ArgPos("-group-size", argc, argv, false)) > 0) group_size = unsigned(atoi(argv[i + 1]));
  labelProcess(input_files, output_files, min_count, max_count, rate, random_flag, label_flag, sort_by_length, group_size);
  return 0;
}
